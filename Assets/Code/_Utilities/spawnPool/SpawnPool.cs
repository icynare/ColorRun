...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        /// <param name="needCheck">need to check is PrefabPool allready in</param>
        /// <returns>A List of instances spawned or an empty List</returns>
        public void CreatePrefabPool(PrefabPool prefabPool, bool needCheck = true)
        {
            ...
            //   Awake();
            bool isAlreadyPool = !needCheck || this.GetPrefab(prefabPool.prefab) == null ? false : true;
            if (!isAlreadyPool)
            {
        ...
        public PrefabPool CreatePrefabPool(Transform prefab)
        {
            PrefabPool newPrefabPool = new PrefabPool(prefab);
            this.CreatePrefabPool(newPrefabPool, false);
...
//            PrefabPool newPrefabPool = new PrefabPool(prefab);
//            this.CreatePrefabPool(newPrefabPool, false);
            PrefabPool newPrefabPool = CreatePrefabPool(prefab);
